...
def parse_input_parameter():
    ...
    try:
        ...
        # Create a Namespace with all arguments defined in the parser
...
def save_model_no_epoch(args, model):
    ...
    model_to_save = model.module if hasattr(model, 'module') else model
    output_model_file = os.path.join(
        args.output_dir, "post_train_pytorch_model.bin")
    torch.save(model_to_save.state_dict(), output_model_file)
    logger.info("Model saved to %s", output_model_file)
    return output_model_file
...
# specifying the pathnames for the different datasets
DATALOADER_DICT = {}
...
def main():
    ...
    if args.do_train:
        ...
        ## This validation file is taken from the path
        ...
        for epoch in range(args.num_train_epochs):
            ...
            # NOTE: eval_dataloader refers to the validation set
            p, r, f1  = eval_epoch(model, eval_dataloader, label_list, device)

            # if f1 > best_f1:
                # print("Best F1 Score: ", f1, " at epoch: ", epoch, " saving model...")
                # best_f1 = f1
                # best_epoch = epoch
                # save_model(epoch, args, model)
                
                ## save txt file with the results
            logger.info("F1 Score: %f, Precision: %f, Recall: %f at epoch: %d", f1, p, r, epoch)
...
if __name__ == "__main__":
    try:
        config_paths = [
            "/home/esg_acaa/code/baselines/GRACE/configs/aspect_action/ate_ghl_run_config.json",
            "/home/esg_acaa/code/baselines/GRACE/configs/aspect_action/ate_ghl_vat_run_config.json"
        ]
        # remember to run the following in the command line: CUDA_VISIBLE_DEVICES=0
        # config_path = "/home/esg_acaa/code/baselines/GRACE/configs/aspect_action/ate_lap_vat_run_config"
        # config_path = "/home/esg_acaa/code/baselines/GRACE/configs/aspect_action/ate_ghl_vat_run_config.json"
