...
class PreTrainedBertModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, *inputs, **kwargs):
        ...
        # If user passed a repo id like "owner/name", fetch a local snapshot
        from huggingface_hub import hf_hub_download, snapshot_download
        if isinstance(archive_file, str) and "keentomato/" in archive_file and not os.path.exists(archive_file):
            # Download the repo snapshot to a local dir
            try:
                archive_file = snapshot_download(archive_file)  # add token=.../revision=... as needed
            except Exception:
                # Fallback: try to grab just the two files we need
                tmpdir = tempfile.mkdtemp()
                cfg = hf_hub_download(pretrained_model_name, filename="config.json")
                wts = hf_hub_download(pretrained_model_name, filename="pytorch_model.bin")
                shutil.copy(cfg, os.path.join(tmpdir, "bert_config.json"))  # rename for legacy loader
                shutil.copy(wts, os.path.join(tmpdir, "pytorch_model.bin"))
                archive_file = tmpdir
        ...
        # Load config
        CONFIG_CANDIDATES = ("config.json", "bert_config.json")
        # after you set `serialization_dir`, replace config loading with this:
        config_file = None
        for name in CONFIG_CANDIDATES:
            p = os.path.join(serialization_dir, name)
            if os.path.exists(p):
                config_file = p
                break

        if config_file is None:
            # last-resort: fetch config.json directly from the Hub
            config_file = hf_hub_download(pretrained_model_name, filename="config.json")
