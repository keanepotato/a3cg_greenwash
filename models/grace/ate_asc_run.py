...
def parse_input_parameter():
    ...
    # Optional: let this be set via config too
    parser.add_argument("--prediction_file_path", default=None, type=str,
                        help="CSV path to save predictions during eval (usually provided in config).")
    ...
    # --- Load config JSON and overlay onto args ---
    if args.config_path:
        ...
        args.prediction_file_path = config["prediction_file_path"]

    # --- Validate required fields AFTER config load ---
    must_have = ["data_dir", "bert_model", "output_dir"]
    missing = [k for k in must_have if not getattr(args, k, None)]
    if missing:
        raise ValueError(f"Missing required parameters after config load: {missing}")
...
def save_model_no_epoch(args, model):
    # Only save the model it-self
    model_to_save = model.module if hasattr(model, 'module') else model
    output_model_file = os.path.join(
        args.output_dir, "best_val_score_pytorch_model.bin")
...
def init_model(args, num_tp_labels, task_config, device, n_gpu):
    ...
    if args.init_model:
        ...
        if "keentomato/" in args.init_model:
            model = BertForSequenceLabeling.from_pretrained(
                args.init_model,
                num_tp_labels=(3, 4),
                task_config={
                    "use_ghl": True,
            ...
            print("Loaded from huggingface:", type(model))
...
def dataloader_val(args, tokenizer, file_path, label_tp_list, set_type="val"):
    ...
    # extract all samples
    sample_texts = [example.text_a for example in dataset.examples]

    return eval_dataloader, eval_data, sample_texts
...
def cal_f1_at(y_true, y_pred):
    ...
    for ground_seq, pred_seq in zip(y_true, y_pred):
        ...
        if len(lab_chunks) == 0 and len(lab_pred_chunks) == 0:
            correct_pred += 1
            total_pred += 1
            total_ground += 1
        else:
...
def cal_f1_action(y_true, y_pred):
    total_act_tp = 0
    total_act_pred = 0
    total_act_ground = 0

    for true_seq_lst, pred_seq_lst in zip(y_true, y_pred):
        unmatched_pred_seq_lst = pred_seq_lst.copy()

        # empty ground truth will not iterate
        for gt in true_seq_lst:
            # skip over empty ground truth
            if gt:
                total_act_ground += 1

        # empty predictions will not iterate
        for p in pred_seq_lst:
            # skip over empty predictions
            if p:
                total_act_pred += 1
        ...
        for gt in true_seq_lst:
            # empty predictions will not iterate
            for i, pred in enumerate(unmatched_pred_seq_lst):
                if gt and gt == pred:
                    unmatched_pred_seq_lst.pop(i)
                    total_act_tp += 1
                    break

    p = total_act_tp / total_act_pred if total_act_pred > 0 else 0.
    r = total_act_tp / total_act_ground if total_act_ground > 0 else 0.
    f1 = 2 * p * r / (p + r) if p > 0 and r > 0 else 0.
    return p, r, f1
...
def cal_f1_as(y_true, y_pred, at_lab_chunks_list, at_lab_pred_chunks_list, must_predict=False):
    ...
    for ground_seq, pred_seq, lab_chunks, lab_pred_chunks in zip(y_true, y_pred, at_lab_chunks_list, at_lab_pred_chunks_list):
        ...
        # the action terms correspond directly to the aspect terms
        # because essentially, in the loop, we are confining the 
        # action terms to the aspect terms. 
        # this is correct
        # NOTE: should also count when both are no aspect terms and empty 
        if len(lab_chunks) == 0 and len(lab_pred_chunks) == 0:
            correct_pred += 1
            total_pred += 1
            total_ground += 1
...
def eval_epoch(model, eval_dataloader, label_tp_list, device, test_examples=None, csv_save_path=None):
    ...
    for input_ids, input_mask, segment_ids, at_label_ids, as_label_ids, label_mask, label_mask_X in eval_dataloader:
        ...
        # the logits from the encoder depict the aspect terms; the logits from the decoder depict the aspect sentiment
        ...
        for i, mask_i in enumerate(label_mask):
            ...
            for j, l in enumerate(mask_i):
                if l > -1:
                    ...
                    temp_12.append(at_label_map[logits[i][j]]) #the predictions from the encoder which depict the aspect terms
                    temp_21.append(as_label_map[as_label_ids[i][j]])
                    temp_22.append(as_label_map[decoder_logits[i][j]]) #the predictions from the decoder which depict the aspect sentiment

            y_true_at.append(temp_11)
            y_pred_at.append(temp_12)
            y_true_as.append(temp_21)
            y_pred_as.append(temp_22)
    ## "[['O', 'O', 'B-AP', 'O', 'O', 'O']]" # aspect terms
    ## "[['POSITIVE', 'POSITIVE', 'POSITIVE', 'POSITIVE', 'POSITIVE', 'POSITIVE']]" # aspect sentiment
    # at is in the format of the different aspect terms; as is in the format of the different polarity labels for the different aspect terms
    # calculates the f1 score seperately and not for determining both the aspect terms and the polarity labels correctly in a single tuplet

    # NOTE: CALCULATION OF ASPECT TERMS
    ate_p, ate_r, ate_f1, at_lab_chunks_list, at_lab_pred_chunks_list = cal_f1_at(y_true_at, y_pred_at)
    logger.info("AT p:{:.4f}\tr:{:.4f}\tf1:{:.4f}".format(ate_p, ate_r, ate_f1))

    full_p, full_r, full_f1, true_labels, pred_labels = cal_f1_as(y_true_as, y_pred_as, at_lab_chunks_list, at_lab_pred_chunks_list)
    logger.info("AS p:{:.4f}\tr:{:.4f}\tf1:{:.4f}".format(full_p, full_r, full_f1))

    act_p, act_r, act_f1 = cal_f1_action(true_labels, pred_labels)

    # Assuming test_examples, y_true_as, and y_pred_as are lists
    data = zip(test_examples, true_labels, pred_labels)

    if csv_save_path:
        # check if the directory exists, if not create it
        csv_save_dir = os.path.dirname(csv_save_path)
        ...
        with open(csv_save_path, 'w', newline='') as f:
            writer = csv.writer(f)
            # Write the headers
            writer.writerow(["Sentence", "True Labels", "Predicted Labels"])
            # Write the rows
            writer.writerows(data)
    ate_metrics = {"ate_p": ate_p, "ate_r": ate_r, "ate_f1": ate_f1}
    action_metrics = {"act_p": act_p, "act_r": act_r, "act_f1": act_f1}


    return full_p, full_r, full_f1, ate_metrics, action_metrics
...
DATASET_DICT={}
DATASET_DICT["disclosures"] = {"train_file":"disclosures_train.txt", "valid_file":"disclosures_trial.txt", "test_file":"disclosures_test.txt"}
...
def main():
    ...
    test_dataloader, test_examples, test_samples = DATALOADER_DICT[task_name]["eval"](args, tokenizer, file_path,
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            p, r, f1 = eval_epoch(model, eval_dataloader, label_tp_list, device)

            if f1 > best_f1:
                print("Best Val F1 Score: ", f1, " at epoch: ", epoch, " saving model...")
                best_f1 = f1
                best_epoch = epoch
                # save_model(epoch, args, model)
                save_model_no_epoch(args, model)
                ## save txt file with the results
                logger.info("Best F1 Score: %f at epoch: %d, with Precision: %f, Recall: %f", best_f1, best_epoch, p, r)

        logger.info("***Results on test***")
        eval_epoch(model, test_dataloader, label_tp_list, device)
    elif args.do_eval:
        # feed in the test examples
        p, r, f1, ate_metrics, action_metrics = eval_epoch(model, 
                              test_dataloader, 
                              label_tp_list, 
                              device, 
                              test_examples=test_samples,
                              csv_save_path=args.prediction_file_path)

         # Save results to a text file
        if 'unseen' in args.data_dir:
            eval_results_file = 'unseen_eval_results.txt'
        elif 'seen' in args.data_dir:
            eval_results_file = 'seen_eval_results.txt'
        else:
            eval_results_file = 'eval_results.txt'
        with open(os.path.join(args.output_dir, eval_results_file), 'w') as file:
            file.write(f"Test Precision: {p}, Test Recall: {r}, Test F1: {f1}\n")
            file.write(f"Aspect Term Extraction Metrics: {ate_metrics}\n")
            file.write(f"Action Metrics: {action_metrics}\n")
